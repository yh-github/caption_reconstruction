...
As presented in \Cref{fig:surprisal_immunity} and \Cref{tab:correlations}, prior caption surprisal (APCS NLL) exhibits a massive negative correlation with Phi-3-mini performance (\(r = -0.6408, p < 0.0001\)). Compact models lean heavily on high-probability stereotypical phrases; when real-world events diverge from standard templates, their generative capability collapses.
In sharp contrast, Llama-3.1-8B shows zero correlation with APCS NLL (\(r = +0.0276, p = 0.7908\)). Its parameter scale grants it robust world knowledge, allowing it to in-fill uncommon events with equal precision. Consequently, Llama's margin of victory over Phi-3 increases significantly as caption surprisal rises (\(r = +0.2022, p = 0.0495\)).

\begin{figure}[t]
  \centering
  \includegraphics[width=0.98\columnwidth]{figures/fig3_category_performance.png}
  \vspace{-2mm}
  \caption{\textbf{Reconstruction Performance Across Domains.} Llama-3.1-8B consistently outperforms Phi-3-mini and Visual Continuity across all categories, achieving its largest gains in procedural and action domains.}
  \label{fig:category_performance}
  \vspace{-3mm}
\end{figure}

\subsection{Finding 2: The Modality Decoupling Law}
\Cref{fig:modality_decoupling} empirically validates the Modality Decoupling Law. For non-parametric visual interpolation, motion variance acts as an immediate failure mode: visual cosine similarity degrades dramatically with motion variance (\(r = -0.8648, p < 0.0001\)). When a scene undergoes rapid panning, cutting, or erratic subject displacement, visual interpolation fails because the optical manifold breaks temporal smoothness.
...
\caption{\textbf{Domain Stratification.} Performance across video categories (\(W=3\)). Llama-3.1-8B delivers large, consistent advantages over both visual and small-language baselines.}
...
\begin{figure}[t]
  \centering
  \includegraphics[width=0.75\columnwidth]{figures/fig4_positional_mrr.png}
  \vspace{-2mm}
  \caption{\textbf{Positional Reconstruction MRR.} Opening sequences (\(i=0\)) achieve the highest accuracy due to backward teleological causality in human video editing.}
  \label{fig:positional_mrr}
  \vspace{-3mm}
\end{figure}

\noindent \textbf{Positional Teleology:} Counterintuitively, reconstructing opening sequences (\(i=0\)) yields the highest retrieval MRR (\(0.585 \pm 0.014\)), surpassing middle (\(0.532 \pm 0.014\)) and ending segments (\(0.558 \pm 0.014\)) (\Cref{fig:positional_mrr}). In natural video editing, downstream narrative resolution is strongly determined by initial conditions; working backward from the established premise provides tighter causal constraints than forward open-ended branching.

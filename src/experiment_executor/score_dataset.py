...
# from reconstruction.masking import get_masking_strategies # Removed for Prior-only scoring
...
class ScoringResources:
    ...
    data_loader: Any 
    # masking_strategies: list[Any]
    syncer: HFResultsSync
    existing_data: dict[str, Any]
    model_key: str
    videos_to_process: list[Any]

def parse_scoring_args(arg_list: list[str] | None = None) -> argparse.Namespace:
...
def setup_scoring_resources(args: argparse.Namespace, force_pull: bool = False) -> ScoringResources:
    """
    ...
    hyperparams = {
        "model_key": model_key,
        # "masking_configs": config.get("masking_configs"), # Removed
        "data_config": config.get("data_config"),
        # "seed": config.get("base_params", {}).get("master_seed") # Removed
    }
    # sort_keys=True ensures recursive sorting of nested dict keys (e.g. data_config)
    ...
    # Using 'scoring_default' to separate from training runs if needed, or just standardizing.
...
def refresh_resources(resources: ScoringResources, args: argparse.Namespace, force_pull: bool = False):
    """
...
def upload_checkpoint(syncer_ref: HFResultsSync, existing: dict[str, Any], current_results: dict[str, Any], curr_config: dict[str, Any], count: int):
...
def run_scoring_loop(resources: ScoringResources, args: argparse.Namespace):
    """
    ...
    try:
        for i, video in enumerate(videos_to_process):
            ...
            # B. PMI for Masked Segments -> REMOVED
